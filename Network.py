...
class ResNet(object):
    ...
    def __call__(self, inputs, training):
        ...
        outputs = self._start_layer(inputs, training) # [batch_size, 32, 32, first_num_filters=16]
    ...
    def _batch_norm_relu(self, inputs, training):
        ...
        ### YOUR CODE HERE
        with tf.variable_scope('batch_norm_relu', reuse=tf.AUTO_REUSE):
            outputs = tf.layers.batch_normalization(inputs=inputs, axis=3, training=training, trainable=training, momentum=0.998)
    ...
    def _start_layer(self, inputs, training):
        ...
        with tf.variable_scope('start_layer', reuse=tf.AUTO_REUSE):
            # test_bn = tf.layers.batch_normalization(inputs=inputs, axis=3, training=training, trainable=training)
            outputs = tf.layers.conv2d(inputs=inputs,
                                       filters=self.first_num_filters,
                                       kernel_size=7,   # parameter to be tuned
                                       strides=1,
                                       padding="same",
                                       use_bias=False,
    ...
    def _output_layer(self, inputs, training):
        ...
        with tf.variable_scope('output_layer', reuse=tf.AUTO_REUSE):
            outputs = tf.reduce_mean(inputs, axis=[1, 2])  # shape: [batch_size, num_filters]
    ...
    def _stack_layer(self, inputs, filters, block_fn, strides, training):
        ...
        with tf.variable_scope('stack_layer_' + str(filters), reuse=tf.AUTO_REUSE):
            for i in range(self.resnet_size):
                with tf.variable_scope('block_' + str(i), reuse=tf.AUTO_REUSE):
                    if i == 0:
                        outputs = block_fn(inputs=inputs, filters=filters, training=training, \
                                           projection_shortcut=projection_shortcut, strides=strides)
                    else:
                        outputs = block_fn(inputs=outputs, filters=filters, training=training, \
    ...
    def _standard_block_v1(self, inputs, filters, training, projection_shortcut, strides):
        ...
        if projection_shortcut is not None:
            ### YOUR CODE HERE
            with tf.variable_scope('shortcut_v1', reuse=tf.AUTO_REUSE) as scsc1:
                shortcut = projection_shortcut(shortcut)
                shortcut = tf.layers.batch_normalization(inputs=shortcut, axis=3, trainable=training, training=training, momentum=0.998)

             ### END CODE HERE

        ### YOUR CODE HERE
        with tf.variable_scope('block_v1', reuse=tf.AUTO_REUSE) as scb1:
            outputs = tf.layers.conv2d(inputs=inputs,
                                       filters=filters,
                                       kernel_size=3,
                                       strides=strides,
            ...
            outputs = tf.layers.conv2d(inputs=outputs,
                                       ...
                                       use_bias=False,
                                       kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            outputs = tf.layers.batch_normalization(inputs=outputs, axis=3, training=training, trainable=training, momentum=0.998)
    ...
    def _bottleneck_block_v2(self, inputs, filters, training, projection_shortcut, strides):
        ...
        shortcut = inputs
        if projection_shortcut is not None:
            with tf.variable_scope('shortcut_v2', reuse=tf.AUTO_REUSE) as scsc2:
                shortcut = projection_shortcut(inputs)

        with tf.variable_scope('block_v2', reuse=tf.AUTO_REUSE) as scb2:
            # outputs = self._batch_norm_relu(inputs, training)
            outputs = tf.layers.conv2d(inputs=inputs,   # outputs,
                                       filters=filters,
                                       kernel_size=1,
                                       strides=strides,
                                       padding="same",
                                       use_bias=False,
                                       kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            with tf.variable_scope('bnr_0', reuse=tf.AUTO_REUSE):
                outputs = self._batch_norm_relu(outputs, training)
            outputs = tf.layers.conv2d(inputs=outputs,
                                       filters=filters,
                                       kernel_size=3,
                                       ...
                                       use_bias=False,
                                       kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            with tf.variable_scope('bnr_1', reuse=tf.AUTO_REUSE):
                outputs = self._batch_norm_relu(outputs, training)
            outputs = tf.layers.conv2d(inputs=outputs,
                                       filters=filters * 4,
                                       kernel_size=1,
